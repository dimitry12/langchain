"""Test numpy-based vectorstore functionality."""
...
def test_numpy() -> None:
    ...
    texts = ["foo", "bar", "baz"]
    docsearch = NumpyVectorStore.from_texts(texts, FakeEmbeddings())
...
def test_numpy_with_metadatas() -> None:
    """Test end to end construction and search."""
    texts = ["foo", "bar", "baz"]
    metadatas = [{"page": i} for i in range(len(texts))]
    docsearch = NumpyVectorStore.from_texts(texts, FakeEmbeddings(), metadatas=metadatas)
    expected_docstore = InMemoryDocstore(
        {
...
def test_numpy_search_not_found() -> None:
...
def test_numpy_add_texts_not_supported() -> None:
    """Test adding of texts to a docstore that doesn't support it."""
    docsearch = NumpyVectorStore(FakeEmbeddings().embed_query, None, Wikipedia(), {})
    with pytest.raises(ValueError):
        docsearch.add_texts(["foo"])
